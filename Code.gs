...
function createGameEntrySheet(ss) {
  ...
  // Instructions
  sheet.getRange('D1').setValue('← Fill in game details, then click ⚾ Softball > Save Game')
  ...
  // Player rows for stats - auto-populate names from roster
...
function createDashboardSheet(ss) {
  ...
  sheet.getRange('A2').setValue('Click ⚾ Softball > Refresh Dashboard to update')
    .setFontColor('#666666').setFontStyle('italic');
...
function createLineupSuggesterSheet(ss) {
  ...
  sheet.getRange('A1').setValue('Lineup Suggester')
    .setFontSize(16).setFontWeight('bold');
  sheet.getRange('A2').setValue('Click ⚾ Softball > Suggest Lineup to generate')
    .setFontColor('#666666').setFontStyle('italic');
...
function createHowToUseSheet(ss) {
  ...
  const instructions = [
    ...
    ['5. Save the game', 'Click ⚾ Softball menu > Save Game (saves both lineup and batting stats)'],
    ['', ''],
    ...
    ['3. Click ⚾ Softball > Suggest Lineup', 'The algorithm will generate field positions AND a batting order'],
  ...
  // Section header rows
  const sectionRows = [3, 8, 15, 22, 29, 35];
